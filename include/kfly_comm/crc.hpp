...
{
/**
 * @brief   CRC-CCITT table.
 */
const constexpr std::array< uint16_t, 256 > crc16_table = {
 ...
 */
inline uint16_t generateCRC(const std::vector< uint8_t > &payload,
...
constexpr uint16_t generateCRC(const std::array< uint8_t, N > &payload,
                               const uint16_t crc_start = 0xffff)
{
  uint16_t crc = crc_start;
